...
public:
    ...
    virtual int InitDbConn(const dbagent::tagDbConfDetail& stDbConfDetail) = 0;

    // 本应统一使用InitDbConn(const dbagent::tagDbConnInfo& stDbConnInfo)的，
    // 因数据访问数据库安全需要，走数据代理的连接不传输DB连接信息
//    virtual int InitDbConn(const dbagent::tagAgentNode& stAgentNode)
//    {
//    	return 0;
//    }
