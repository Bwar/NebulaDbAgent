...
public:
    ...
    //初始化连接实例
    virtual int InitDbConn(const dbagent::tagDbConfDetail& stDbConfDetail);
...
public:
     ...
     * dbagent::CMysqlDbi::InitDbConn(const dbagent::tagDbConfDetail& stDbConfDetail)
     * 中的使用。
     */
    ...
    dbagent::T_mapRow& GetRowFromFile();
...
private:
    ...
    dbagent::T_vecDict m_vecDict;	//结果集数据字典
    dbagent::T_mapRow m_mapRow;	//结果集行

    dbagent::tagDbConfDetail m_stDbConfDetail; // 数据库连接信息
 };
