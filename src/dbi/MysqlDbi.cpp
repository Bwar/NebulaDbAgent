        ...
        unsigned int uiPort)
{
	...
	m_stDbConfDetail.m_ucDbType = dbagent::MYSQL_DB;
...
int dbagent::CMysqlDbi::InitDbConn(const dbagent::tagDbConfDetail& stDbConfDetail)
{
	FreeResult();
	InitVariables();
...
int dbagent::CMysqlDbi::MysqlInit()
{
    if (!mysql_init(&m_dbInstance))
    {
        m_iErrno = -1;
...
int dbagent::CMysqlDbi::SetMysqlOptions(
...
int dbagent::CMysqlDbi::MysqlRealConnect(
        const char* szIp,
        const char* szUserName,
        const char* szUserPwd,
        const char* szDbName,
...
int dbagent::CMysqlDbi::SetDbConnCharacterSet(const char* szCharacterSet)
{
    int ret = 0;
...
int dbagent::CMysqlDbi::EscapeString(
        char* szTo,
        const char* szFrom,
...
int dbagent::CMysqlDbi::MysqlPing()
{
    return mysql_ping(&m_dbInstance);
}

int dbagent::CMysqlDbi::SetAutoCommit(my_bool mode)
{
    int ret = 0;
...
int dbagent::CMysqlDbi::ExecSql(const std::string& strSql)
{
...
unsigned int dbagent::CMysqlDbi::AffectRows()
{
	return(mysql_affected_rows(&m_dbInstance));
}

const MYSQL_RES* dbagent::CMysqlDbi::StoreResult()
{
...
const MYSQL_RES* dbagent::CMysqlDbi::UseResult()
{
	if (m_iErrno != 0)  // 查询是失败的
	{
	    return NULL;
...
int dbagent::CMysqlDbi::GetResultSet(T_vecResultSet& vecResultSet)
{
	vecResultSet.clear();
	if (m_iQueryResult == 0)
    {
...
int dbagent::CMysqlDbi::GetResultToFile()
{
	 //查询失败，取不了结果集
	 if (m_iQueryResult != 0)
	 {
...
dbagent::T_mapRow& dbagent::CMysqlDbi::GetRowFromFile()
{
	m_mapRow.clear();
...
int dbagent::CMysqlDbi::GetResultDict(T_vecDict& vecDict)
{
	m_vecDict.clear();
...
int dbagent::CMysqlDbi::GetResultRow(T_mapRow& mapRow)
{
	mapRow.clear();
...
MYSQL_ROW dbagent::CMysqlDbi::GetRow()
{
    if (m_pQueryRes == NULL)
    {
        return NULL;
...
unsigned long* dbagent::CMysqlDbi::FetchLengths()
{
    if (m_pQueryRes != NULL)
    {
...
unsigned int dbagent::CMysqlDbi::FetchFieldNum()
{
    if (m_pQueryRes != NULL)
    {
...
MYSQL_FIELD* dbagent::CMysqlDbi::FetchFields()
{
    if (m_pQueryRes != NULL)
    {
...
unsigned int dbagent::CMysqlDbi::FetchFieldCount()
{
    return mysql_field_count(&m_dbInstance);
}

void dbagent::CMysqlDbi::FreeResult()
{
    if (m_pQueryRes != NULL)
    {
...
const char* dbagent::CMysqlDbi::MysqlInfo()
{
    return mysql_info(&m_dbInstance);
}

void dbagent::CMysqlDbi::InitVariables()
{
	m_iQueryResult = 0;
	m_iErrno = 0;
	m_pQueryRes = NULL;
...
bool dbagent::CMysqlDbi::CleanBeforeExecSql()
{
	//测试数据库连接的可用性
